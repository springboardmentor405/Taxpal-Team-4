...
const COLORS = ["#6366f1","#ef4444","#f97316","#22c55e","#a855f7","#06b6d4"];
...
export default function Dashboard() {
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh" }}>
      ...
      <div style={{ marginLeft: 240, flex: 1, padding: "16px 20px", background: "#f1f5f9" }}>

        {/* ── Header ── */}
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <h4 style={{ fontWeight: 700, fontSize: 22, margin: 0 }}>Financial Dashboard</h4>
          <div style={{ display: "flex", gap: 12 }}>
            <button onClick={() => setModal("income")} style={{
              background: "none", border: "none",
              color: "#4f46e5", fontWeight: 600, fontSize: 14, cursor: "pointer"
            }}>
              +Record Income
            </button>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14, marginBottom: 16 }}>
          {[
            ...
            { label: "Net Balance",   value: netBalance,   color: netBalance >= 0 ? "#1e3a8a" : "#dc2626", bg: "#ede9fe", icon: "💰" },
          ].map(card => (
            <div key={card.label} style={{
              ...
              <div style={{ fontSize: 12, color: "#6b7280", marginBottom: 4 }}>{card.icon} {card.label}</div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 16 }}>
          ...
          {/* Bar Chart */}
          <div style={{ background: "white", borderRadius: 12, padding: 16, boxShadow: "0 2px 8px rgba(0,0,0,.06)" }}>
            <h6 style={{ fontWeight: 600, marginBottom: 12, textAlign: "center", fontSize: 14 }}>
              Income vs Expenses
            </h6>
            {loading ? (
        ...
        {/* ── Recent Transactions ── */}
        <div style={{ background: "white", borderRadius: 12, padding: 16, boxShadow: "0 2px 8px rgba(0,0,0,.06)" }}>
          <h6 style={{ fontWeight: 600, marginBottom: 12, textAlign: "center", fontSize: 15 }}>
            Recent Transactions
          </h6>
          {loading ? (
            <div style={{ textAlign: "center", color: "#9ca3af", padding: "24px 0" }}>Loading...</div>
          ) : recent.length === 0 ? (
              ...
              <thead>
                <tr style={{ borderBottom: "2px solid #f1f5f9", background: "#fafafa" }}>
                  {["Date","Description","Category","Amount","Type"].map(h => (
                    <th key={h} style={{ padding: "10px 14px", textAlign: "left", fontWeight: 700, fontSize: 13 }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {recent.map((tx, i) => (
                  ...
                  >
                    ...
                    <td style={{ padding: "12px 14px", fontSize: 13, fontWeight: 500 }}>{tx.description}</td>
                    <td style={{ padding: "12px 14px", fontSize: 13, color: "#6b7280" }}>{tx.category}</td>
      ...
      {modal && <TransactionModal type={modal} onClose={() => setModal(null)} onSave={load} />}
    </div>
  );
}